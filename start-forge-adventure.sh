...
echo "Waiting for X server :100 (Adventure Forge)..."
for i in {1..30}; do
    if xdpyinfo -display :100 >/dev/null 2>&1; then
        echo "X server :100 is ready"
        break
    fi
    sleep 1
done
...
echo "Starting Adventure Mode Forge instance..."
...
if [ -n "$JAR_FILE" ] && [ -f "$JAR_FILE" ]; then
    echo "Starting Adventure Mode Forge using $JAR_FILE on display :100..."
